...
//Update an existing notes using :PUT  "/api/notes/updatenote" .
router.put(
...
//Delete an existing notes using :DELETE  "/api/notes/deletenote" .
router.delete(
  "/deletenote/:id",fetchuser,async (req, res) => {
    ...
    //Allow deletion only for owner of the note
    if(note.user.toString() !== req.user.id){
      return res.status(401).send("Not Allowed");
    }
    // Update the note with the new data
